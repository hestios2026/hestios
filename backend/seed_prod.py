...
Creates admin user from env vars + imports real company data.
...
ADMIN_EMAIL    = os.environ.get("ADMIN_EMAIL", "admin@hesti-rossmann.de")
ADMIN_PASSWORD = os.environ.get("ADMIN_PASSWORD", "")
ADMIN_NAME     = os.environ.get("ADMIN_NAME", "Alexandru Dumitrache")
...
# ── Kostenstellen (real company data) ─────────────────────────────────────────
kostenstellen = [
...
for ks, name, client in kostenstellen:
    if not db.query(Site).filter(Site.kostenstelle == ks).first():
        ...
        inserted += 1
print(f"✓ Kostenstellen: {inserted} new, {len(kostenstellen) - inserted} already exist")
...
for product, unit, *supplier_prices in prices:
    for sup_name, price in zip(["Eberle Hald","Niklaus Baugeräte","Würth","HKL","Förch"], supplier_prices):
        ...
        if not sup or not sup.id:
            continue
        if not db.query(SupplierPrice).filter(
